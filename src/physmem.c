 ...
 */
static kword_t phys_read(paddr_t paddr, unsigned width) {
 ...
 */
static void phys_write(paddr_t paddr, kword_t value, unsigned width) {
	struct IOPCIDiagnosticsParameters param;
	param.spaceType      = kIOPCI64BitMemorySpace;
	param.bitWidth       = width * 8;
	param.options        = 0;
 ...
 * 	addresses.
 */
static const kword_t kernel_virtual_to_physical_mask = 0xffffffff;
